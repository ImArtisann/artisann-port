...
import { DEPLOYED_PROJECTS, projectPreviewUrl } from "./projects.ts";
...
const fetchAllowed = Effect.fn("ProjectPreview.fetch")(function* (
    ...
    for (let redirects = 0; redirects <= 3; redirects++) {
        ...
        if (response.status >= 300 && response.status < 400 && response.headers.location) {
            url = new URL(response.headers.location, checked).href;
            continue;
        }
...
const downloadPreview = Effect.fn("ProjectPreview.download")(function* (
    project: (typeof DEPLOYED_PROJECTS)[number],
) {
    const pageHost = new URL(project.href).hostname;
    const page = yield* fetchAllowed(project.href, [pageHost, pageHost.replace(/^www\./, "")]);
    if (!page.response.headers["content-type"]?.includes("text/html")) {
        return yield* new PreviewError({ message: "Project did not return HTML" });
    }
    const html = yield* readBounded(page.response, 1_048_576);
    const imageReference = yield* extractOpenGraphImage(html);
    const imageUrl = yield* Effect.try({
        try: () => allowedUrl(imageReference, page.url, project.imageHosts).href,
        catch: () => new PreviewError({ message: "Unsafe Open Graph image URL" }),
    });
    const image = yield* fetchAllowed(imageUrl, project.imageHosts);
...
export const projectPreviewResponse = Effect.fn("ProjectPreview.respond")(function* (
    ...
    const project = DEPLOYED_PROJECTS.find(
        (candidate) => path === `/projects/${candidate.id}/og-image`,
    );
    if (!project) return new Response(null, { status: 404 });
    // Ignore arbitrary query parameters and request headers when choosing a cache key.
    const key = new Request(projectPreviewUrl(project));
    const existing = yield* Effect.tryPromise(() => cache.match(key));
    const response =
        existing ??
        (yield* downloadPreview(project).pipe(
